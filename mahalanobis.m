...
%   each vector in Y to tha mean (centroid) of the vectors in X, and
%   outputs the result in vector D, whose length is size(Y, 1). The vectors
%   in X and Y are assumed to be organized as rows. The
%   input data can be real or complex. The outputs are real
%   quantities.
%
%   D = MAHALANOBIS(Y, XC MX) computes the Mahalanobis distnace between
...
%   real quantities. In addition to the mean vector MX, the covariance
%   matrix CX of a population of vectors X must be
%   provided also. Use funtcion COVMATRIX (Section 12.5) to compute
%   MX and CX.
%
% Reference: Acklam, P. J. [2002]. "MATLAB Array Manipulation Tips
% and Tricks, " available at
%   home.online.no/~jacklam/matlab/doc/mtt/index.html
